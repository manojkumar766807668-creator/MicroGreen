...
export default function Contact() {
  ...
  const cards = [[MessageCircle, "WhatsApp", c.whatsapp && waLink(c.whatsappGreeting), c.phone], [Phone, "Phone", c.phone && `tel:${c.phone}`, c.phone], [Mail, "Email", c.email && `mailto:${c.email}`, c.email],
  ...
  return <><PageHero title="Say hello." /><section className="sec"><div className="wrap cgrid">{cards.filter(([, , , v]) => v).map(([I, t, href, v]) => {
    const inner = <><I aria-hidden="true" /><h3>{t}</h3><p>{v}</p></>;
