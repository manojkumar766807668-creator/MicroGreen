...
const formatGrowthTime = (growthTime) => {
  if (!growthTime) return "";
  if (typeof growthTime === "string") return growthTime;

  const timelineText = Array.isArray(growthTime.timeline)
    ? growthTime.timeline.map((item) => `${item.day}: ${item.stage} — ${item.description}`).join(" • ")
    : "";

  return [growthTime.total, timelineText, growthTime.note].filter(Boolean).join(" ");
};
...
export default function ProductDetails() {
  ...
  const rows = [
    ...
    ["Nutrition", p.nutrients.join(", ")],
    ["Growing time", formatGrowthTime(p.growthTime)],
    ["Freshness / shelf life", p.shelfLife],
    ["Storage", p.storage],
    ["Best ways to use", p.bestWaysToUse.join(", ")]
  ];
