...
export default function ProductDetails() {
  ...
  return <section className="sec pd"><div className="wrap pd-in">
    ...
    <div><p className="small"><Link to="/products">← All products</Link></p><h1>{p.name}</h1>{p.taste && <p className="muted">{p.taste}</p>}<p>{p.description}</p>
    ...
    <div className="wrap"><dl className="facts pd-facts">{rows.filter(([, v]) => v && v !== req).map(([k, v]) => <div key={k}><dt>{k}</dt><dd>{v}</dd></div>)}</dl></div></section>;
}
