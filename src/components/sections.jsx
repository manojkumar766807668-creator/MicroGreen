...
export function WhatIs() {
  ...
  return <section className="sec"><div className="wrap split">
    <Reveal><h2 className="giant">Tiny plants.<br />Big stage.</h2></Reveal>
    <Reveal delay={100}><p className="lead">Microgreens are young vegetable and herb plants, harvested early, while the leaves are still small. (Confirm wording against the BRD.)</p></Reveal></div>
...
const vs = { micro: ["Microgreens", "Grown in a growing medium and cut above it. Eaten as leaf and stem."], sprout: ["Sprouts", "Germinated seeds, usually eaten whole with the seed and root."] };
export function Versus() {
  const [on, setOn] = useState("micro");
  return <section className="sec cream"><div className="wrap"><Heading title="Microgreens vs sprouts">Tap a side to see the difference. (Confirm against the BRD.)</Heading>
    <div className="vs">{Object.entries(vs).map(([k, [t, d]]) => (
      <button key={k} className={`vs-panel ${k} ${on === k ? "on" : ""}`} aria-pressed={on === k} onClick={() => setOn(k)}><h3>{t}</h3><p>{d}</p></button>))}</div></div></section>;
}
...
export function Featured() {
  ...
  return <section className="sec"><div className="wrap"><Heading title="Fresh from the farm">Our featured greens.</Heading>
    ...
    <div className="center"><Btn to="/products" variant="outline">See all products</Btn></div></div></section>;
}
...
export function UseCases({ all }) {
  const list = all ? useCases : useCases.slice(0, 6);
  return <section className="sec"><div className="wrap"><Heading title="A little green goes a long way" />
    <div className="uses">{list.map((u, i) => <Reveal key={u.name} className={`use u${i}`} delay={i * 60}><Img alt={u.name} /><div><h3>{u.name}</h3><p>{u.text}</p><p className="small">Where the greens go: {u.placement}</p><p className="small">Product used: {u.product}</p></div></Reveal>)}</div>
    {!all && <Btn to="/how-to-use">Discover More Ways to Use</Btn>}</div></section>;
}
